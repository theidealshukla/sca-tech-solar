        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <h2 className="font-black text-night-900 leading-tight mb-6" style={{ fontSize: 'clamp(2rem, 4.5vw, 4rem)', letterSpacing: '-0.03em' }}>
              ...
              <span className="font-display italic text-solar-500">Indore families & businesses.</span>
            </h2>
            <div className="flex items-center gap-4 mb-8">
              <div className="flex -space-x-3">
                {['RS', 'PA', 'AP', 'SV'].map((init, i) => (
                  <div key={i} className={`w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-white text-xs font-bold ${['bg-solar-500','bg-sky-500','bg-purple-500','bg-green-500'][i]}`}>
                    {init}
                  </div>
                ))}
              </div>
              <div>
                <div className="flex">
                  {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 text-solar-500 fill-solar-500" />)}
                </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="w-11 h-11 rounded-full bg-night-900 text-white flex items-center justify-center hover:bg-solar-500 transition-all duration-300"
              >
          ...
          <div className="relative">
            <div className="bg-white rounded-3xl p-8 md:p-10 shadow-xl border border-night-100">
              <Quote className="w-8 h-8 text-solar-200 mb-6" />
              ...
              <div className="mt-6 pt-6 border-t border-night-100 grid grid-cols-2 gap-4">
                ...
                <div className="bg-solar-50 rounded-xl p-3">
                  <p className="text-xs text-solar-600 mb-1">Result</p>
                  <p className="font-bold text-solar-700 text-sm">{testimonials[active].saving}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center gap-2 mt-5">
              {testimonials.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActive(i)}
                  className={`transition-all duration-300 rounded-full ${i === active ? 'w-8 h-2 bg-solar-500' : 'w-2 h-2 bg-night-300'}`}
                />
              ))}
            </div>
          </div>
